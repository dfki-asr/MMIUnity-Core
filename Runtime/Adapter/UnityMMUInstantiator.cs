    ...
    public class UnityMMUInstantiator : IMMUInstantiation
    {
        ...
        public IMotionModelUnitDev InstantiateMMU(MMULoadingProperty mmuLoadingProperty)
        {
            ...
            MainThreadDispatcher.Instance.ExecuteBlocking(delegate
            {
                ...
                try
                {
                    if (!mmuLoadingProperty.Data.ContainsKey(assetBundlePath))
                    {
                        ...
                        if (mmuAssetBundle == null)
                        {
                            ...
                            MMICSharp.Logger.Log(Log_level.L_ERROR, "Failed to load AssetBundle! AssetBundle is null!" + mmuDescription.Name);
                            return;
                        }
                ...
                {
                    MMICSharp.Logger.Log(Log_level.L_ERROR, "Cannot load asset bundle: " + mmuDescription.Name + " " + e.Message);
                    return;
                }
                ...
                {
                    MMICSharp.Logger.Log(Log_level.L_ERROR, "Cannot load prefab: " + mmuDescription.Name + " " + e.Message);
                }
                ...
                {
                    MMICSharp.Logger.Log(Log_level.L_ERROR, "Cannot instantiate prefab: " + mmuDescription.Name + " " + e.Message);
                }
                ...
                try
                {
                    ...
                    if (classType != null)
                    {
                        ...
                        {
                            MMICSharp.Logger.Log(Log_level.L_DEBUG, $"MMU {mmuDescription.Name} implements the UnityMMUBase");
                            ...
                            if (this.GetLocalAvatarConfigurationFile(folderPath, out string localConfigFilePath))
                            {
                                ...
                                MMICSharp.Logger.Log(Log_level.L_DEBUG, $"Local avatar configuration was found: {mmuDescription.Name} ({avatarConfigFilePath}).");
                            ...
                            if (System.IO.File.Exists(avatarConfigFilePath))
                            {
                                ...
                                MMICSharp.Logger.Log(Log_level.L_DEBUG, $"Setting the configuration file path for loading the avatar of {mmuDescription.Name} to {avatarConfigFilePath}");
                            }

                            //Problem in here -> File is obviously not available
                            else
                            {
                                MMICSharp.Logger.Log(Log_level.L_ERROR, $"Problem setting the configuration file path for loading the avatar of {mmuDescription.Name} to {avatarConfigFilePath}. File not available.");
                            }
                        }
                    ...
                    {
                        MMICSharp.Logger.Log(Log_level.L_ERROR, "Cannot instantiate MMU: " + mmuDescription.Name);
                    }
                }
                catch (Exception e)
                {
                    MMICSharp.Logger.Log(Log_level.L_ERROR, "Problem at instantiating class of MMU: " + mmuDescription.Name + " " + e.Message + " " + e.StackTrace);
                }
